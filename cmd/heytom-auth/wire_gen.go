...
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	...
	grpcServer := server.NewGRPCServer(confServer, greeterService, userService, roleService, policyService, applicationService, logger)
	authRepo := data.NewAuthRepo(dataData)
	authUsecase := biz.NewAuthUsecase(authRepo)
	authService := service.NewAuthService(authUsecase)
	httpServer := server.NewHTTPServer(confServer, greeterService, userService, roleService, policyService, applicationService, authService, logger)
